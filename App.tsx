...
import { ChartIcon, MagicWandIcon, ShopIcon, NewsIcon, MapIcon } from './components/Icons';

type ViewMode = 'generator' | 'shop' | 'stats' | 'news' | 'map';
...
const App: React.FC = () => {
  ...
  const NavButton = ({ mode, icon: Icon, label }: { mode: ViewMode, icon: any, label: string }) => (
      ...
      >
          ...
          <span className="relative z-10 text-xs sm:text-sm whitespace-nowrap">{label}</span>
      </button>
  );
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <div className="sticky top-[80px] z-40 py-6 backdrop-blur-sm">
            <div className="container mx-auto px-4 flex justify-center gap-4 flex-wrap">
                <div className="p-1.5 bg-black/40 backdrop-blur-md border border-white/10 rounded-3xl flex flex-wrap justify-center gap-2 shadow-2xl overflow-x-auto no-scrollbar max-w-full">
